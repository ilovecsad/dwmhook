...
ZwSetInformationThread_(
 ...
 struct  SysCall
{
	DWORD ZwCreateSection;
	DWORD ZwMapViewOfSection;
...
EXTERN_C  ULONG WINAPI shellSysCall64();
VOID InitSysCallIndex(SysCall* pInfo);



DWORD WINAPI ZwCreateSection(PHANDLE            SectionHandle,
	ACCESS_MASK        DesiredAccess,
	POBJECT_ATTRIBUTES ObjectAttributes,
	PLARGE_INTEGER     MaximumSize,
	ULONG              SectionPageProtection,
...
DWORD WINAPI ZwMapViewOfSection(HANDLE          SectionHandle,
	HANDLE          ProcessHandle,
	PVOID* BaseAddress,
	ULONG_PTR       ZeroBits,
	SIZE_T          CommitSize,
...
ULONG WINAPI ZwUnmapViewOfSection(
	_In_ HANDLE ProcessHandle,
	_In_opt_ PVOID BaseAddress
	);
...
NTSTATUS NTAPI ZwCreateThreadEx(PVOID StartRoutine, PVOID StartContext);
...
NTSTATUS NTAPI ZwResumeThread(IN HANDLE ThreadHandle, OUT PULONG PreviousSuspendCount OPTIONAL);
